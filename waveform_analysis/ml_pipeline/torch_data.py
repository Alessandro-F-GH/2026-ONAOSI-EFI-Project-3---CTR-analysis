...
def factored_correction_target_ps(
    ...
    """Relative LED target with native-grid alignment residual removed.

    For detector j:
        delta_j = t_LED,j(interpolated) - t_anchor,j(native)

    The residual pair shift still encoded in the aligned waveform is:
        Delta delta = delta_1 - delta_2

    The waveform model therefore learns:
        g(s1) - g(s2) = Delta t_LED - Delta delta - TOF_true.

    Delta delta is NOT added back analytically at inference.
    """
