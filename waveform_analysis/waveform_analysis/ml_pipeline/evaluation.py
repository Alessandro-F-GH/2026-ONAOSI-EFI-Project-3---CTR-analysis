...
def evaluate_final_test(
    ...
    *,
    ...
    plot_dir = Path(pipeline_config["paths"]["plot_dir"]) / "final_evaluation"
    ...
    swapped_fit = _fit(
        swapped_calibrated, "Energy LED + CNN correction (swapped)", fit_config
    )
    if not swapped_fit.success:
        raise RuntimeError(f"Swap-test Gaussian fit failed: {swapped_fit.message}")
    canonical_fit = fits["Energy LED + CNN correction"]
    swap_test = {
    ...
    output_dir = Path(pipeline_config["paths"]["work_dir"])
    ...
    summary = {
        "comparison": "Energy LED standard vs Energy CFD standard vs Energy LED corrected",
